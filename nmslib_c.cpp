...
#define SET_LAST_ERROR(code, msg) \
    NMSLIBUtil::set_last_error(code, msg, __FILE__, __LINE__)
...
static void nmslib_borrowed_data_free(void* ptr) {
    if (!ptr) return;
    nmslib_borrowed_data_t* borrowed =
        static_cast<nmslib_borrowed_data_t*>(ptr);
...
inline void set_last_error(nmslib_error_t code, const std::string& msg,
                           const char* file, int line) {
    last_error_detail = {code, msg.empty() ? "No error" : msg, file, line};
}

inline char* dup_string(const std::string& str,
...
inline nmslib_error_t validate_common_inputs(nmslib_index_handle_t index,
...
inline nmslib_error_t validate_pointer_batch(const void* const* data_ptrs,
                                             size_t count,
...
inline nmslib_error_t validate_sparse_elements(
    const nmslib_sparse_elem_float_t* elements, size_t num_elements,
    bool sorted) {
...
struct nmslib_internal_index_t {
    ...
    nmslib_internal_index_t(const std::string& m, const std::string& st,
                            nmslib_data_type_t dt, nmslib_dist_type_t dst,
                            const nmslib_allocator_t* alloc)
        : header{dt, dst},
          space(),
          index_ptr(),
          data(),
...
auto dispatch_index_by_data_type(nmslib_index_handle_t handle, Fn&& fn)
    -> std::invoke_result_t<Fn, nmslib_internal_index_t<float>*> {
    ...
    switch (hdr->data_type) {
        ...
        case NMSLIB_DATATYPE_SPARSE_VECTOR: {
            auto idx =
                reinterpret_cast<nmslib_internal_index_t<float>*>(handle);
...
// Consolidated helper for creating query/data objects
template <typename dist_t>
std::unique_ptr<Object> create_object(const Space<dist_t>* space,
                                      nmslib_data_type_t data_type,
                                      const void* data, size_t elem_count,
                                      size_t num_elements, int32_t id) {
    std::unique_ptr<Object> obj;
    switch (data_type) {
        case NMSLIB_DATATYPE_DENSE_VECTOR: {
            const float* vec = static_cast<const float*>(data);
            auto vectSpace =
                dynamic_cast<const VectorSpaceSimpleStorage<float>*>(space);
            if (!vectSpace) return nullptr;
            std::vector<float> tempVec(vec, vec + elem_count);
            obj.reset(vectSpace->CreateObjFromVect(
                static_cast<IdType>(id), static_cast<LabelType>(-1), tempVec));
            break;
        }
        case NMSLIB_DATATYPE_SPARSE_VECTOR: {
            const nmslib_sparse_elem_float_t* elems =
                static_cast<const nmslib_sparse_elem_float_t*>(data);
            if (nmslib_error_t err = NMSLIBUtil::validate_sparse_elements(
                    elems, num_elements, true)) {
            ...
            auto sparseSpace =
                dynamic_cast<const SpaceSparseVectorSimpleStorage<float>*>(
                    space);
            if (!sparseSpace) return nullptr;
            obj.reset(sparseSpace->CreateObjFromVect(
                static_cast<IdType>(id), static_cast<LabelType>(-1), tempVec));
            break;
        }
        case NMSLIB_DATATYPE_DENSE_UINT8_VECTOR: {
            ...
            obj.reset(siftSpace->CreateObjFromUint8Vect(
                static_cast<IdType>(id), static_cast<LabelType>(-1), u8vec));
            break;
        }
        case NMSLIB_DATATYPE_OBJECT_AS_STRING: {
            const char* str_data = static_cast<const char*>(data);
            std::string str(str_data, elem_count > 0 ? elem_count - 1
                                                     : std::strlen(str_data));
            auto temp_obj = space->CreateObjFromStr(static_cast<IdType>(id),
...
void extract_knn_results(const KNNQueue<dist_t>* res, nmslib_result_t* result) {
    ...
    if (found > result->capacity) {
        SET_LAST_ERROR(NMSLIB_ERROR_BUFFER_TOO_SMALL,
                       "Result buffers too small for " + std::to_string(found));
        result->size = 0;
        return;
    }
...
nmslib_error_t nmslib_index_create(
    ...
    if (!space || !method || !allocator || !allocator->alloc ||
        !allocator->free || !out_handle) {
    ...
    try {
        {
            void* mem = allocator->alloc(sizeof(nmslib_internal_index_t<float>),
                                         allocator->ctx);
            if (!mem) {
            ...
            auto idx = new (mem) nmslib_internal_index_t<float>(
                method, space, data_type, dist_type, allocator);
            bool created = false;
            try {
                idx->space.reset(
                    SpaceFactoryRegistry<float>::Instance().CreateSpace(
                        space,
                        load_params(reinterpret_cast<nmslib_params_wrapper_t*>(
                            space_params))));
                if (idx->space) {
                    *out_handle = reinterpret_cast<nmslib_index_handle_t>(idx);
                    SET_LAST_ERROR(NMSLIB_SUCCESS,
                                   "Index created using float registry");
                    return NMSLIB_SUCCESS;
                }
        ...
        {
            void* mem = allocator->alloc(sizeof(nmslib_internal_index_t<int>),
                                         allocator->ctx);
            if (!mem) {
                SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
                               "Failed to allocate index (fallback)");
                return NMSLIB_ERROR_OUT_OF_MEMORY;
            }

            auto idx_i = new (mem) nmslib_internal_index_t<int>(
                method, space, data_type, dist_type, allocator);
            try {
                idx_i->space.reset(
                    SpaceFactoryRegistry<int>::Instance().CreateSpace(
                        space,
                        load_params(reinterpret_cast<nmslib_params_wrapper_t*>(
                            space_params))));
            } catch (const std::exception& e) {
                ...
                allocator->free(mem, allocator->ctx);
                SET_LAST_ERROR(
                    NMSLIB_ERROR_SPACE_INCOMPATIBLE,
                    std::string("Failed to create space (int fallback): ") +
                        e.what());
                return NMSLIB_ERROR_SPACE_INCOMPATIBLE;
            } catch (...) {
                idx_i->~nmslib_internal_index_t();
                ...
                SET_LAST_ERROR(
                    NMSLIB_ERROR_SPACE_INCOMPATIBLE,
                    "Failed to create space (int fallback unknown error)");
                return NMSLIB_ERROR_SPACE_INCOMPATIBLE;
            }
            ...
            if (!idx_i->space) {
                ...
                SET_LAST_ERROR(
                    NMSLIB_ERROR_SPACE_INCOMPATIBLE,
                    "No compatible space found in float or int registry");
                return NMSLIB_ERROR_SPACE_INCOMPATIBLE;
            }

            *out_handle = reinterpret_cast<nmslib_index_handle_t>(idx_i);
            SET_LAST_ERROR(NMSLIB_SUCCESS,
                           "Index created using int registry fallback");
            return NMSLIB_SUCCESS;
        }
        ...
        SET_LAST_ERROR(NMSLIB_ERROR_RUNTIME,
                       "Failed to create index: " + std::string(e.what()));
...
nmslib_error_t nmslib_create_index(nmslib_index_handle_t handle,
    ...
    return dispatch_index_by_data_type(
        handle, [index_params, print_progress](auto* idx) -> nmslib_error_t {
            ...
            try {
                ...
                idx->index_ptr.reset(factory.CreateMethod(
                    print_progress != 0, idx->method, idx->space_type,
                    *idx->space, idx->data));
                idx->index_ptr->CreateIndex(load_params(
                    reinterpret_cast<nmslib_params_wrapper_t*>(index_params)));
                SET_LAST_ERROR(NMSLIB_SUCCESS, "Index created successfully");
                return NMSLIB_SUCCESS;
            } catch (const std::bad_alloc& e) {
                SET_LAST_ERROR(
                    NMSLIB_ERROR_OUT_OF_MEMORY,
                    "Memory allocation failed: " + std::string(e.what()));
                return NMSLIB_ERROR_OUT_OF_MEMORY;
            } catch (const std::exception& e) {
                SET_LAST_ERROR(
                    NMSLIB_ERROR_INDEX_BUILD_FAILED,
                    "Failed to create index: " + std::string(e.what()));
                return NMSLIB_ERROR_INDEX_BUILD_FAILED;
            } catch (...) {
                SET_LAST_ERROR(NMSLIB_ERROR_INDEX_BUILD_FAILED,
                               "Failed to create index (unknown error)");
                return NMSLIB_ERROR_INDEX_BUILD_FAILED;
            }
        });
...
nmslib_params_handle_t nmslib_create_params(
    ...
    try {
        void* mem =
            allocator->alloc(sizeof(nmslib_params_wrapper_t), allocator->ctx);
        if (!mem) {
            SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
...
nmslib_error_t nmslib_add_param(nmslib_params_handle_t params, const char* name,
    ...
    try {
        auto params_wrapper =
            reinterpret_cast<nmslib_params_wrapper_t*>(params);
        std::string param = std::string(name) + "=";
        switch (type) {
            case 0:
                param += std::to_string(*static_cast<const int*>(value));
                break;
            case 1:
                param += std::to_string(*static_cast<const double*>(value));
                break;
            case 2:
                param += static_cast<const char*>(value);
                break;
            default:
                SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
...
void nmslib_free_params(nmslib_params_handle_t params) {
    if (!params ||
        !reinterpret_cast<nmslib_params_wrapper_t*>(params)->allocator.free) {
        SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                       "Invalid params or allocator");
        return;
    }
    auto params_wrapper = reinterpret_cast<nmslib_params_wrapper_t*>(params);
    params_wrapper->~nmslib_params_wrapper_t();
    params_wrapper->allocator.free(params_wrapper,
                                   params_wrapper->allocator.ctx);
}

nmslib_error_t nmslib_get_space_type(nmslib_index_handle_t handle,
                                     const char** space_type,
                                     size_t* space_type_len,
                                     const nmslib_allocator_t* allocator) {
    if (!handle || !space_type || !space_type_len || !allocator ||
    ...
    return dispatch_index_by_data_type(
        handle,
        [space_type, space_type_len, allocator](auto* idx) -> nmslib_error_t {
            try {
                *space_type_len = idx->space_type.length();
                *space_type =
                    NMSLIBUtil::dup_string(idx->space_type, allocator);
                if (!*space_type) {
                    SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
                                   "Failed to allocate memory for space type");
                    return NMSLIB_ERROR_OUT_OF_MEMORY;
                }
                SET_LAST_ERROR(NMSLIB_SUCCESS,
                               "Space type retrieved successfully");
                return NMSLIB_SUCCESS;
            } catch (...) {
                SET_LAST_ERROR(NMSLIB_ERROR_RUNTIME,
...
nmslib_error_t nmslib_get_method(nmslib_index_handle_t handle,
                                 const char** method, size_t* method_len,
                                 const nmslib_allocator_t* allocator) {
    if (!handle || !method || !method_len || !allocator || !allocator->alloc ||
        !allocator->free) {
        SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT, "Invalid arguments");
        return NMSLIB_ERROR_INVALID_ARGUMENT;
    }
    return dispatch_index_by_data_type(
        handle, [method, method_len, allocator](auto* idx) -> nmslib_error_t {
            try {
                *method_len = idx->method.length();
                *method = NMSLIBUtil::dup_string(idx->method, allocator);
                if (!*method) {
                    SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
                                   "Failed to allocate memory for method");
                    return NMSLIB_ERROR_OUT_OF_MEMORY;
                }
...
nmslib_error_t nmslib_get_last_error_detail(
    ...
    if (!detail || !allocator || !allocator->alloc || !allocator->free) {
        SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                       "Invalid detail or allocator pointer");
        return NMSLIB_ERROR_INVALID_ARGUMENT;
    }
    try {
        detail->code = last_error_detail.code;
        detail->message =
            NMSLIBUtil::dup_string(last_error_detail.message, allocator);
        if (!detail->message) {
            SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
        ...
        detail->file =
            NMSLIBUtil::dup_string(last_error_detail.file, allocator);
        if (!detail->file) {
            allocator->free(const_cast<char*>(detail->message), allocator->ctx);
            SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
                           "Failed to allocate memory for error file");
            return NMSLIB_ERROR_OUT_OF_MEMORY;
        }
...
nmslib_error_t nmslib_add_data_point(nmslib_index_handle_t handle,
                                     const void* data, size_t element_count,
                                     int32_t id) {
    if (nmslib_error_t err =
            NMSLIBUtil::validate_common_inputs(handle, data, element_count)) {
        SET_LAST_ERROR(err, "Invalid inputs for adding data point");
        return err;
    }
    return dispatch_index_by_data_type(
        handle, [data, element_count, id](auto* idx) -> nmslib_error_t {
            try {
                auto obj = create_object(
                    idx->space.get(), idx->data_type, data, element_count,
                    idx->data_type == NMSLIB_DATATYPE_SPARSE_VECTOR
                        ? element_count
                        : 0,
                    id);
                if (!obj) {
...
nmslib_error_t nmslib_add_data_point_batch(nmslib_index_handle_t index,
    ...
    return dispatch_index_by_data_type(
        index,
        [data, count, element_count, ids,
         num_elements](auto* idx) -> nmslib_error_t {
            try {
                size_t offset = 0;
                for (size_t i = 0; i < count; ++i) {
                    int32_t curr_id_int =
                        ids ? ids[i] : static_cast<int32_t>(i);
                    ...
                    if (idx->data_type == NMSLIB_DATATYPE_SPARSE_VECTOR) {
                        stride = curr_num_elements *
                                 sizeof(nmslib_sparse_elem_float_t);
                        effective_dim = 0;
                    } else if (idx->data_type ==
                               NMSLIB_DATATYPE_DENSE_UINT8_VECTOR) {
                        stride = element_count * sizeof(uint8_t);
                    } else {
                        stride = element_count * sizeof(float);
                    }
                    const void* curr_data =
                        static_cast<const char*>(data) + offset;
                    auto obj = create_object(idx->space.get(), idx->data_type,
                                             curr_data, effective_dim,
                                             curr_num_elements, curr_id_int);
                    if (!obj) {
                        SET_LAST_ERROR(NMSLIB_ERROR_RUNTIME,
                                       "Failed to create batch object");
                        return NMSLIB_ERROR_RUNTIME;
                    }
                    idx->data.push_back(obj.release());
                    offset += stride;
                ...
                SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
                               "Batch alloc failed: " + std::string(e.what()));
                return NMSLIB_ERROR_OUT_OF_MEMORY;
            } catch (...) {
...
nmslib_error_t nmslib_add_data_point_batch_uint8(nmslib_index_handle_t handle,
    ...
    if (!handle || !data || count == 0 || element_count == 0) {
        SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                       "Invalid uint8 batch inputs");
        return NMSLIB_ERROR_INVALID_ARGUMENT;
    }
    return dispatch_index_by_data_type(
        handle, [data, count, element_count, ids](auto* idx) -> nmslib_error_t {
            try {
                if (idx->data_type != NMSLIB_DATATYPE_DENSE_UINT8_VECTOR) {
                    SET_LAST_ERROR(NMSLIB_ERROR_SPACE_INCOMPATIBLE,
                                   "Not uint8 vector space");
                    return NMSLIB_ERROR_SPACE_INCOMPATIBLE;
                }
                auto siftSpace =
                    dynamic_cast<const SpaceL2SqrSift*>(idx->space.get());
                if (!siftSpace) {
                    SET_LAST_ERROR(NMSLIB_ERROR_SPACE_INCOMPATIBLE,
                                   "Not SIFT space");
                    return NMSLIB_ERROR_SPACE_INCOMPATIBLE;
                ...
                for (size_t i = 0; i < count; ++i) {
                    ...
                    std::vector<uint8_t> tempVec(vec_start,
                                                 vec_start + element_count);
                    Object* raw_obj = siftSpace->CreateObjFromUint8Vect(
                        static_cast<IdType>(curr_id_int),
                        static_cast<LabelType>(-1), tempVec);
                    if (!raw_obj) {
                        SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
                                       "Failed to create uint8 object");
                        return NMSLIB_ERROR_OUT_OF_MEMORY;
                    }
                    idx->data.push_back(raw_obj);
                }
                SET_LAST_ERROR(NMSLIB_SUCCESS,
                               "UInt8 batch added successfully");
                return NMSLIB_SUCCESS;
            } catch (const std::bad_alloc& e) {
                SET_LAST_ERROR(
                    NMSLIB_ERROR_OUT_OF_MEMORY,
...
nmslib_error_t nmslib_add_data_point_batch_string(nmslib_index_handle_t index,
    ...
    return dispatch_index_by_data_type(
        index, [data, count, ids](auto* idx) -> nmslib_error_t {
            try {
                for (size_t i = 0; i < count; ++i) {
                    if (!data[i]) {
                        SET_LAST_ERROR(NMSLIB_ERROR_NULL_POINTER,
                                       "Null string in batch");
                        return NMSLIB_ERROR_NULL_POINTER;
                    }
                    std::string str(data[i]);
                    std::unique_ptr<Object> obj(idx->space->CreateObjFromStr(
                        ids ? ids[i] : static_cast<int32_t>(i), -1, str,
                        nullptr));
                    if (!obj) {
                        SET_LAST_ERROR(NMSLIB_ERROR_RUNTIME,
                ...
                SET_LAST_ERROR(NMSLIB_SUCCESS,
                               "String batch added successfully");
                return NMSLIB_SUCCESS;
            } catch (...) {
                SET_LAST_ERROR(NMSLIB_ERROR_RUNTIME,
                               "Failed to add string batch");
                return NMSLIB_ERROR_RUNTIME;
            }
        });
}
...
nmslib_error_t nmslib_knn_query_fill(nmslib_index_handle_t index,
                                     ...
                                     size_t num_elements) {
    if (nmslib_error_t err = NMSLIBUtil::validate_common_inputs(
            index, query, query_size_or_elem_count, result)) {
    ...
    return dispatch_index_by_data_type(
        index,
        [query, query_size_or_elem_count, k, result,
         num_elements](auto* idx) -> nmslib_error_t {
            using dist_t = decltype(idx->space->IndexTimeDistance(
                static_cast<const Object*>(nullptr),
                static_cast<const Object*>(nullptr)));
            ...
            try {
                size_t effective_num_elements =
                    (idx->data_type == NMSLIB_DATATYPE_SPARSE_VECTOR)
                        ? num_elements
                        : 0;
                auto qobj = create_object(idx->space.get(), idx->data_type,
                                          query, query_size_or_elem_count,
                                          effective_num_elements, 0);
                if (!qobj) {
                    SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                ...
                SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
                               "Query alloc failed: " + std::string(e.what()));
                return NMSLIB_ERROR_OUT_OF_MEMORY;
            } catch (...) {
                SET_LAST_ERROR(NMSLIB_ERROR_QUERY_EXECUTION_FAILED,
                               "KNN query failed");
...
nmslib_error_t nmslib_knn_query_batch(nmslib_index_handle_t index,
    ...
    if (!index || !queries || query_count == 0 || !results) {
        SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                       "Invalid batch knn inputs");
        return NMSLIB_ERROR_INVALID_ARGUMENT;
    }
    try {
        for (size_t i = 0; i < query_count; ++i) {
            nmslib_error_t err = nmslib_knn_query_fill(
                index,
                static_cast<const char*>(queries) +
                    i * query_size_or_elem_count * sizeof(float),
                query_size_or_elem_count, k, &results[i],
                num_elements ? num_elements[i] : 0);
        ...
        SET_LAST_ERROR(NMSLIB_ERROR_QUERY_EXECUTION_FAILED,
                       "Failed to execute batch knn query");
        return NMSLIB_ERROR_QUERY_EXECUTION_FAILED;
    }
}
...
nmslib_error_t nmslib_range_query_get_size(nmslib_index_handle_t index,
    ...
    if (!index || !query || radius < 0 || !out_size) {
        SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                       "Invalid range query inputs");
        return NMSLIB_ERROR_INVALID_ARGUMENT;
    }
    // Return a modest estimate; range_query_fill will set the actual number in
    // result->size.
    *out_size = 128;
...
nmslib_error_t nmslib_range_query_fill(nmslib_index_handle_t index,
    ...
    if (!index || !query || !result || result->capacity == 0) {
        SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                       "Invalid range fill inputs");
        return NMSLIB_ERROR_INVALID_ARGUMENT;
    }

    return dispatch_index_by_data_type(
        index,
        [query, query_size_or_elem_count, radius, result,
         num_elements](auto* idx) -> nmslib_error_t {
            using dist_t = decltype(idx->space->IndexTimeDistance(
                static_cast<const Object*>(nullptr),
                static_cast<const Object*>(nullptr)));
            ...
            try {
                ...
                if (!res_ptr) {
                    result->size = 0;
                    SET_LAST_ERROR(NMSLIB_SUCCESS,
                                   "Range query returned zero results");
                    return NMSLIB_SUCCESS;
                }

                size_t n = std::min(res_ptr->size(), result->capacity);
                size_t i = 0;
                for (auto it = res_ptr->begin(); it != res_ptr->end() && i < n;
                     ++it, ++i) {
                    const Object* obj = *it;
                    result->ids[i] = static_cast<int32_t>(obj->id());
                    result->distances[i] = idx->space->IndexTimeDistance(
                        rangeQuery.QueryObject(), obj);
                }
                result->size = n;

                SET_LAST_ERROR(NMSLIB_SUCCESS,
                               "Range query filled successfully");
                return NMSLIB_SUCCESS;
                ...
                if (what.find("Range search") != std::string::npos ||
                    ...
                    SET_LAST_ERROR(
                        NMSLIB_ERROR_SPACE_INCOMPATIBLE,
                        std::string("Range query not supported by method: ") +
                            what);
                    return NMSLIB_ERROR_SPACE_INCOMPATIBLE;
                }
...
nmslib_error_t nmslib_get_distance(nmslib_index_handle_t index, size_t pos1,
    ...
    if (!index || pos1 >= nmslib_data_qty(index) ||
        pos2 >= nmslib_data_qty(index) || !distance) {
        SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                       "Invalid distance inputs");
        return NMSLIB_ERROR_INVALID_ARGUMENT;
    }
    return dispatch_index_by_data_type(
        index, [pos1, pos2, distance](auto* idx) -> nmslib_error_t {
            try {
                *distance = idx->space->IndexTimeDistance(idx->data[pos1],
                                                          idx->data[pos2]);
                SET_LAST_ERROR(NMSLIB_SUCCESS,
                               "Distance computed successfully");
                return NMSLIB_SUCCESS;
            } catch (...) {
                SET_LAST_ERROR(NMSLIB_ERROR_RUNTIME,
...
nmslib_error_t nmslib_get_data_point_size(nmslib_index_handle_t index,
    ...
    if (!index || position >= nmslib_data_qty(index) || !size) {
        SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                       "Invalid data point size inputs");
        return NMSLIB_ERROR_INVALID_ARGUMENT;
    }
    return dispatch_index_by_data_type(
        index, [position, size](auto* idx) -> nmslib_error_t {
            try {
...
nmslib_error_t nmslib_get_data_point_fill(nmslib_index_handle_t index,
    ...
    if (!index || !data || size == 0 || position >= nmslib_data_qty(index)) {
        SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                       "Invalid data point fill inputs");
        return NMSLIB_ERROR_INVALID_ARGUMENT;
    }
    return dispatch_index_by_data_type(
        index, [position, data, size](auto* idx) -> nmslib_error_t {
            try {
                const void* src = idx->data[position]->data();
                size_t src_size = idx->data[position]->datalength();
                if (size < src_size) {
                    SET_LAST_ERROR(NMSLIB_ERROR_BUFFER_TOO_SMALL,
                                   "Buffer too small for data point");
...
nmslib_error_t nmslib_get_data_point_string(
    ...
    if (!index || !data || !data_len || !allocator ||
        position >= nmslib_data_qty(index)) {
        SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                       "Invalid string data point inputs");
        return NMSLIB_ERROR_INVALID_ARGUMENT;
    }
    return dispatch_index_by_data_type(
        index,
        [position, data, data_len, allocator](auto* idx) -> nmslib_error_t {
            try {
                if (idx->data_type != NMSLIB_DATATYPE_OBJECT_AS_STRING) {
                    SET_LAST_ERROR(NMSLIB_ERROR_SPACE_INCOMPATIBLE,
                                   "Invalid data type for string");
                    return NMSLIB_ERROR_SPACE_INCOMPATIBLE;
                }
                const char* str =
                    static_cast<const char*>(idx->data[position]->data());
                *data_len = idx->data[position]->datalength() + 1;
                *data = NMSLIBUtil::dup_string(std::string(str, *data_len - 1),
                                               allocator);
                if (!*data) {
                    SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
                ...
                SET_LAST_ERROR(NMSLIB_ERROR_RUNTIME,
                               "Failed to get string data point");
                return NMSLIB_ERROR_RUNTIME;
            }
        });
}
...
nmslib_error_t nmslib_borrow_data_dense(nmslib_index_handle_t index,
    ...
    if (!index || !data || !size || !free_fn ||
        position >= nmslib_data_qty(index)) {
        SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                       "Invalid dense borrow inputs");
        return NMSLIB_ERROR_INVALID_ARGUMENT;
    }
    return dispatch_index_by_data_type(
        index, [position, data, size, free_fn](auto* idx) -> nmslib_error_t {
            try {
                ...
                nmslib_borrowed_data_t* borrowed =
                    static_cast<nmslib_borrowed_data_t*>(idx->allocator.alloc(
                        sizeof(nmslib_borrowed_data_t), idx->allocator.ctx));
                if (!borrowed) {
                    SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
                                   "Failed to allocate wrapper");
                    return NMSLIB_ERROR_OUT_OF_MEMORY;
                }
                *size = idx->data[position]->bufferlength();
                void* tempData =
                    idx->allocator.alloc(*size, idx->allocator.ctx);
                if (!tempData) {
                    idx->allocator.free(borrowed, idx->allocator.ctx);
                    SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
                                   "Failed to allocate data copy");
                    return NMSLIB_ERROR_OUT_OF_MEMORY;
                }
    ...
    return dispatch_index_by_data_type(
        index, [position, data, size, free_fn](auto* idx) -> nmslib_error_t {
            try {
                ...
                *size = idx->data[position]->bufferlength() /
                        sizeof(nmslib_sparse_elem_float_t);
                void* tempData = idx->allocator.alloc(
                    *size * sizeof(nmslib_sparse_elem_float_t),
                    idx->allocator.ctx);
                if (!tempData) {
                    idx->allocator.free(borrowed, idx->allocator.ctx);
                    SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
                                   "Failed to allocate sparse copy");
                    return NMSLIB_ERROR_OUT_OF_MEMORY;
                }
                std::memcpy(tempData, idx->data[position]->data(),
                            *size * sizeof(nmslib_sparse_elem_float_t));
                borrowed->data = tempData;
                borrowed->allocator = idx->allocator;
                *data = tempData;
                *free_fn = nmslib_borrowed_data_free;
                ...
                SET_LAST_ERROR(NMSLIB_ERROR_RUNTIME,
                               "Failed to borrow sparse data");
                return NMSLIB_ERROR_RUNTIME;
            }
        });
}

nmslib_error_t nmslib_save_index(nmslib_index_handle_t handle, const char* path,
                                 int save_data) {
    if (!handle || !path) {
    ...
    return dispatch_index_by_data_type(
        handle, [path, save_data](auto* idx) -> nmslib_error_t {
            try {
                if (!idx->index_ptr) {
                    SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                                   "Index not built");
                ...
                if (save_data) {
                    ...
                    idx->space->WriteObjectVectorBinData(
                        idx->data, dummy, std::string(path) + ".dat");
                }
                idx->index_ptr->SaveIndex(path);
                SET_LAST_ERROR(NMSLIB_SUCCESS, "Index saved successfully");
                return NMSLIB_SUCCESS;
            } catch (...) {
                SET_LAST_ERROR(NMSLIB_ERROR_DATA_IO_FAILED,
...
nmslib_error_t nmslib_load_index(const char* path, nmslib_data_type_t data_type,
    ...
    if (!path || !allocator || !allocator->alloc || !allocator->free ||
        !out_handle) {
    ...
    try {
        ...
        switch (dist_type) {
            case NMSLIB_DISTTYPE_FLOAT: {
                mem = allocator->alloc(sizeof(nmslib_internal_index_t<float>),
                                       allocator->ctx);
                if (!mem) {
                    SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
                                   "Failed to allocate index");
                ...
                auto idx = new (mem) nmslib_internal_index_t<float>(
                    "hnsw", "l2", data_type, dist_type, allocator);
                idx->space.reset(
                    SpaceFactoryRegistry<float>::Instance().CreateSpace(
                        "l2", AnyParams()));
                ...
                idx->index_ptr.reset(factory.CreateMethod(
                    print_progress, "hnsw", "l2", *idx->space, idx->data));
                if (load_data) {
                    std::vector<std::string> dummy;
                    idx->space->ReadObjectVectorFromBinData(
                        idx->data, dummy, std::string(path) + ".dat");
                }
            ...
            case NMSLIB_DISTTYPE_INT: {
                mem = allocator->alloc(sizeof(nmslib_internal_index_t<int>),
                                       allocator->ctx);
                if (!mem) {
                    SET_LAST_ERROR(NMSLIB_ERROR_OUT_OF_MEMORY,
                                   "Failed to allocate index");
                ...
                auto idx = new (mem) nmslib_internal_index_t<int>(
                    "hnsw", "l2", data_type, dist_type, allocator);
                idx->space.reset(
                    SpaceFactoryRegistry<int>::Instance().CreateSpace(
                        "l2", AnyParams()));
...
nmslib_error_t nmslib_set_query_time_params(nmslib_index_handle_t handle,
    ...
    return dispatch_index_by_data_type(
        handle, [params](auto* idx) -> nmslib_error_t {
            try {
                if (!idx->index_ptr) {
                    SET_LAST_ERROR(NMSLIB_ERROR_INDEX_BUILD_FAILED,
                                   "Index not built");
                ...
                idx->index_ptr->SetQueryTimeParams(load_params(
                    reinterpret_cast<nmslib_params_wrapper_t*>(params)));
                SET_LAST_ERROR(NMSLIB_SUCCESS, "Query time params set");
                return NMSLIB_SUCCESS;
            } catch (...) {
                SET_LAST_ERROR(NMSLIB_ERROR_RUNTIME,
                               "Failed to set query time params");
                return NMSLIB_ERROR_RUNTIME;
            }
        });
}

nmslib_error_t nmslib_set_thread_pool_size(nmslib_index_handle_t handle,
                                           size_t size) {
    ...
    return dispatch_index_by_data_type(
        handle, [size](auto* idx) -> nmslib_error_t {
            try {
                ...
                SET_LAST_ERROR(NMSLIB_ERROR_RUNTIME,
                               "Failed to set thread pool size");
                return NMSLIB_ERROR_RUNTIME;
            }
        });
}
...
size_t nmslib_get_thread_pool_size(nmslib_index_handle_t handle) {
    ...
    return dispatch_index_by_data_type(
        handle, [](auto* idx) -> size_t { return idx->thread_pool_size; });
}
...
size_t nmslib_data_qty(nmslib_index_handle_t handle) {
    ...
    return dispatch_index_by_data_type(
        handle, [](auto* idx) -> size_t { return idx->data.size(); });
}
...
nmslib_error_t nmslib_add_data_point_batch_pointers(
    ...
    if (!handle || !data_ptrs || count == 0) {
        SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                       "Invalid pointer batch inputs");
        return NMSLIB_ERROR_INVALID_ARGUMENT;
    }
    if (nmslib_error_t err = NMSLIBUtil::validate_pointer_batch(
            data_ptrs, count, element_count)) {
        return err;
    }

    return dispatch_index_by_data_type(
        handle,
        [data_mode, data_ptrs, count, element_count, ids,
         num_elements](auto* idx) -> nmslib_error_t {
            try {
                for (size_t i = 0; i < count; ++i) {
                    const int32_t curr_id =
                        ids ? ids[i] : static_cast<int32_t>(i);
                    std::unique_ptr<Object> obj;
                    size_t curr_num =
                        (data_mode == NMSLIB_DATA_MODE_SPARSE && num_elements)
                            ? num_elements[i]
                            : element_count;
                    size_t effective_dim = element_count;
                    if (data_mode == NMSLIB_DATA_MODE_SPARSE) {
                        ...
                        const nmslib_sparse_elem_float_t* elems =
                            static_cast<const nmslib_sparse_elem_float_t*>(
                                data_ptrs[i]);
                        if (nmslib_error_t err =
                                NMSLIBUtil::validate_sparse_elements(
                                    elems, curr_num, true)) {
                            return err;
                        }
                        ...
                        const uint8_t* u8vec_ptr =
                            static_cast<const uint8_t*>(data_ptrs[i]);
                        std::vector<uint8_t> u8data(u8vec_ptr,
                                                    u8vec_ptr + element_count);
                        auto siftSpace = dynamic_cast<const SpaceL2SqrSift*>(
                            idx->space.get());
                        if (!siftSpace) {
                            SET_LAST_ERROR(NMSLIB_ERROR_SPACE_INCOMPATIBLE,
                                           "Not SIFT uint8 space");
                            return NMSLIB_ERROR_SPACE_INCOMPATIBLE;
                        }
                        Object* raw_obj = siftSpace->CreateObjFromUint8Vect(
                            curr_id, static_cast<LabelType>(-1), u8data);
                        obj.reset(raw_obj);
                    } else {
                        SET_LAST_ERROR(NMSLIB_ERROR_INVALID_ARGUMENT,
                    ...
                    if (!obj && data_mode != NMSLIB_DATA_MODE_UINT8) {
                        obj = create_object(idx->space.get(), idx->data_type,
                                            data_ptrs[i], effective_dim,
                                            curr_num, curr_id);
                    }
                    if (!obj) {
                        SET_LAST_ERROR(
                            NMSLIB_ERROR_RUNTIME,
                            "Failed to create object from data pointer");
                        return NMSLIB_ERROR_RUNTIME;
                    }
                    idx->data.push_back(obj.release());
                }
                SET_LAST_ERROR(NMSLIB_SUCCESS,
                               "Pointer batch added successfully");
                return NMSLIB_SUCCESS;
            } catch (...) {
                SET_LAST_ERROR(NMSLIB_ERROR_RUNTIME,
                               "Failed to add pointer batch");
                return NMSLIB_ERROR_RUNTIME;
            }
        });
}

void nmslib_free_result(nmslib_result_t* result,
                        const nmslib_allocator_t* allocator) {
